...
my $configuration_file = "/etc/smokeping/notify";
    ...
    if ( $tree->{alerts} ) {
        ...
        for (sort { ($cfg->{Alerts}{$a}{priority}||0)  
            ...
            if ($what and (not defined $priority or not defined $priority_done )) {
                ...
                my $telegram_token = $config_hash->{telegram_token};
                my $telegram_chat_id = $config_hash->{telegram_chat_id};
...
DOC
                    ...
                    open (LOG, ">>/var/log/smokeping") or die "$! error trying to overwrite";
...
DOC
                        ...
                        my $telegram_message = fill_template($alert->{mailtemplate},
                        {
                            ALERT => $_,
                            WHAT  => $what,
                            LINE  => $line,
                    ...
                    try {
                            ...
                            my $result = eval { $api->getMe }
                                or die 'Got error message: ', print LOG "\n Telegram error is $api->parse_error->{msg}";

                            $api->sendMessage( {chat_id => $telegram_chat_id, text => $telegram_message, parse_mode => 'Markdown' } );
                            print LOG "\n Send notify to telegram success!";
                    } catch {
                            print LOG "\n Send notify to telegram Failed!";
                    };

                    
                    try {
                            my $slack_hook = $config_hash->{slack_hook_url};

                            my $send_slack = Slack::Notify->new(
                                hook_url => $slack_hook,
                            );
                            ...
                            print LOG "\n Send notify to slack success!";
                    } catch {
                            print LOG "\n Send notify to slack Failed!";
                    };
